...
def cost(t,tprev,alpha,v_thresh):
	...
	v_current=v
	obj =  - alpha*ln(v_current)*dx(domain=mesh) - alpha*ln(v_thresh - v_current)*dx(domain=mesh)
	val = assemble(obj) - (t-tprev)
	# print("---")
	# print(tprev)
	# print(t)
	# print(val)
...
def grad(t,tprev,alpha,v_thresh):
	...
	#time.sleep(.5)
	print((tprev,t))
	mesh2 = IntervalMesh(128, tprev, t+s)
	...
	val1 = -1*dx(domain=mesh2) - alpha*ln(v)*dx(domain=mesh2) - alpha*ln(v_thresh - v)*dx(domain=mesh2)
	val2 = -1*dx(domain=mesh) - alpha*ln(v_current)*dx(domain=mesh) - alpha*ln(v_thresh - v_current)*dx(domain=mesh)
	val = (assemble(val1)-assemble(val2))/s
	#print(val)
	# if(math.isnan(val)):
	# 	#print("hit")
	# 	return 0
